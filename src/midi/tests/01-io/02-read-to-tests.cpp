...
<<<<<<< HEAD
TEST_CASE("read_to<uint_16> from {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x23, 0x34, 0x56} with n == 5")
{
    uint16_t result[5];
    char buffer[] = { 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x23, 0x34, 0x56 };
    std::string data(buffer, sizeof(buffer));
    std::stringstream ss(data);

    io::read_to(ss, result, 5);
    CATCH_CHECK(result[0] == 0x2211);
    CATCH_CHECK(result[1] == 0x4433);
    CATCH_CHECK(result[2] == 0x6655);
    CATCH_CHECK(result[3] == 0x2377);
    CATCH_CHECK(result[4] == 0x5634);
=======
...
>>>>>>> 688be10e085f25371bb266db049da41f369ff49b
}
